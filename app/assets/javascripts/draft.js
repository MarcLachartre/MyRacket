document.addEventListener('turbolinks:load', () => {
  ...
  searchbarCheckboxes.forEach(checkbox => {
    checkbox.addEventListener('change', async() => {
      ...
      const search = await new RacketSearchDisplay(searchbarCheckboxes, searchField, racketsInContainer);
      const displaySearchResult = await search.racketsToUpdate() ;
    });
  });
});
...
class RacketSearch extends FetchDatabase{ //this class creates an object containing the checkboxex and the values of the racket search query so that we can later fetch the database
...
class RacketSearchDisplay extends RacketSearch {
  ...
  racketSearchDifference(racketsArray) { //returns the items that superior amounts contains that are not included in the inferiorAmount
  ...
  addRackets(racketsToAdd) {
    racketsToAdd.forEach(racketObj => {
      const racketCard = new RacketCard
      racketCard.createCard();
    });
  }
