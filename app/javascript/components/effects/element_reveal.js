...
export class ElementReveal {
  constructor(slideLeft, slideRight, slideUp) {
    this.slideLeft = {distance: '400px', duration: 800, easing: 'cubic-bezier(.55,0,.5,.99)', opacity: 0, origin: 'right'};
    this.slideRight = {distance: '400px', duration: 800, easing: 'cubic-bezier(.55,0,.5,.99)', opacity: 0, origin: 'left'};
    this.slideUp = {distance: '300px', duration: 600, easing: 'cubic-bezier(.55,0,.5,.99)', opacity: 0, origin: 'bottom'};
    this.fadeIn = {};
  }

  hasToBeRevealed(selector, selectorType) {
    if (selectorType === "id" && document.getElementById(selector).getBoundingClientRect().y <= 0) {
      return false;
    } else if (selectorType === "class" && document.querySelector(String(selector)).getBoundingClientRect().y <= 0) {
      return false;
    } else {
      return true;
    }
  ...
  listReveal(elementsArray, scrollRevealEffect, elementRevealDelay) { //elementsArray is the list of items to reveal on page, scrollRevealEffect is the type of effect you want (see ScrollReaveal API, defaults), elementRevealDelay is the delay you want to apply between each element reveal.
    const sr = new ScrollReveal();
    ...
    for (let i = 0; i <= elementsArray.length - 1; i++) { // I want to know if element is on the screen, if yes apply style to it.
      ...
      if (elementPositionInContainer < (-elementHeight)) { // this condition retrieves hidden elements above the user position on the container
        elementsAbove.push(elementsArray[i]);
        // console.log(`is above the in container`)
      } else if (elementPositionInWindow > window.innerHeight) { // this condition retrieves hidden elements below the user position on the container
        elementsBelow.push(elementsArray[i]);
        // console.log("is below the container")
      } else { // this condition retrieves the elements visible on the screen
  ...
  const visibleElements = [];
    for (let i = 0; i <= elementsArray.length - 1; i++) { // I want to know if element is on the screen, if yes apply style to it.
      ...
      if (elementPositionInWindow < window.innerHeight && elementPositionInContainer > (-elementHeight)) { // this condition retrieves hidden elements below the user position on the container
        visibleElements.push(elementsArray[i]);
